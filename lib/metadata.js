...
var Metadata = (function () {
    ...
    Metadata.prototype.processMetadata = function (createdTypes) {
        ...
        types.dts = '/*//////////////////////////////////////////////////////////////////////////////////////\n' +
            ...
            '//////                      OData  V4  TypeScript                              /////////\n' +
            '//////////////////////////////////////////////////////////////////////////////////////*/\n\n';
        if (!this.options.ignoreCoreInDts)
        ...
        types.push.apply(types, typeDefinitions.map(function (d) {
            ...
            if (d.baseType == '$data.Enum') {
                ...
                if (d.params[2] && Object.keys(d.params[2]).length > 0) {
                    ...
                    Object.keys(d.params[2]).forEach(function (dp) {
                        var definition = d.params[2][dp];
                        if (definition.type == _this.options.entitySetType && definition.actions) {
                            dtsPart.push('        ' + dp + ': ' + dp + ';');
                            dtsEntitySetPart.push('    export class ' + dp + ' extends $data.EntitySet<typeof ' + definition.elementType + ', ' + definition.elementType + '> {');
                            Object.keys(definition.actions).forEach(function (a) { return dtsEntitySetPart.push('        ' + a + ': ' + _this._typeToTS(definition.actions[a].type, definition.actions[a].elementType, definition.actions[a]) + ';'); });
                            dtsEntitySetPart.push('    }');
                            dtsEntitySetPart.push('');
                        }
                        else {
                            dtsPart.push('        ' + dp + ': ' + _this._typeToTS(definition.type, definition.elementType, definition) + ';');
                        }
                    });
                }
                else
            ...
            types.src += srcPart;
            dtsPart.push('    }');
            dtsPart.push.apply(dtsPart, dtsEntitySetPart);
    ...
    Metadata.prototype._typeToTS = function (type, elementType, definition) {
        ...
        if (type == this.options.entitySetType) {
            ...
            if (!t)
                t = 'Promise<void>';
            else if (t.indexOf('$data.Queryable') < 0)
                t = 'Promise<' + t + '>';
